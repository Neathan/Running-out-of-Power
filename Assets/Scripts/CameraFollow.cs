...
public class CameraFollow : MonoBehaviour {
    ...
    void LateUpdate() {
        ...
        if(focusArea.velocity.x != 0.0f) {
            ...
            if(Mathf.Sign(target.playerInput.x) == Mathf.Sign(focusArea.velocity.x) && target.playerInput.x != 0.0f) {
                ...
                if (!lookAheadStopped) {
                    ...
                    targetLookAheadX = currentLookAheadX + (lookAheadDirectionX * lookAheadDistanceX - currentLookAheadX) / 4.0f;
                }
            }
        }
